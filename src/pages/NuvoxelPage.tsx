  ...
  return (
    <div className="h-full overflow-y-auto p-6 lg:p-8">
      <section className="overflow-hidden rounded-3xl border border-[var(--accent)]/30 bg-gradient-to-br from-[var(--accent)]/20 via-bg-card to-bg-secondary p-7 shadow-2xl">
        <div className="flex flex-wrap items-start justify-between gap-6"><div className="max-w-2xl"><div className="mb-3 flex items-center gap-2 text-sm font-bold text-[var(--accent)]"><ShieldCheck className="h-5 w-5" /> NUVOXEL CLIENT</div><h1 className="text-3xl font-black tracking-tight text-text-primary">Minecraft з можливостями Nuvoxel</h1><p className="mt-3 text-sm leading-6 text-text-secondary">Обери версію — лаунчер автоматично поставить Nuvoxel-мод і Fabric у окремий чистий профіль. Твої звичайні моди та збірки не змінюються й не можуть викликати конфлікт.</p></div><button type="button" onClick={() => void play()} className="inline-flex items-center gap-2 rounded-xl bg-[var(--accent)] px-5 py-3 font-bold text-white shadow-lg transition hover:brightness-110"><Gamepad2 className="h-5 w-5" /> Грати</button></div>
      </section>
